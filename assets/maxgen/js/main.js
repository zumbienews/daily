...
/*===========================================
	=        LoadMore Active        =
=============================================*/
function initLoadMore(selector, findItem, btn, itemsPerPage, display) {
    $(selector).slice(0, itemsPerPage).css('display', display);

    $(btn).on("click", function(e) {
        e.preventDefault();

        $(findItem + ":hidden").slice(0, 2).slideDown(200, function () {
            $(this).css('display', display);
        });

        if ($(findItem + ":hidden").length === 0) {
            $(btn).text("No More Post").addClass("noContent");
        }
    });
}

initLoadMore(".latest__post-wrap .latest__post-item", ".latest__post-item", "#loadMore", 6, "flex");
initLoadMore(".latest__post-wrap .latest__post-item-two", ".latest__post-item-two", "#loadMoreTwo", 5, "block");
initLoadMore(".minimal__post-wrap .minimal__post-item", ".minimal__post-item", "#loadMoreMinimal", 9, "block");
